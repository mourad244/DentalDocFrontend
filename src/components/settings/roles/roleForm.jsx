import React from "react";
import Joi from "joi-browser";
import Form from "../../../common/form";
import { saveRole } from "../../../services/roleService";

class RoleForm extends Form {
  state = {
    data: {
      nom: "",
    },
    formDisplay: false,
    errors: {},
  };
  schema = {
    _id: Joi.string(),
    nom: Joi.string()
      .required()
      .error((errors) => {
        errors.forEach((err) => {
          if (err.type === "any.empty") {
            err.message = "Le champ Rôle est requis.";
          }
        });
        return errors;
      }),
  };

  async populateRoles() {
    try {
      const role = this.props.selectedRole;
      if (role)
        this.setState({
          data: this.mapToViewModel(role),
        });
    } catch (ex) {
      if (ex.response && ex.response.status === 404)
        this.props.history.replace(":not-found");
    }
  }

  async componentDidMount() {
    await this.populateRoles();
  }
  async componentDidUpdate(prevProps, prevState) {
    if (!this.props.selectedRole && prevState.data._id) {
      this.setState({ data: { nom: "" } });
    }
    if (
      this.props.selectedRole &&
      this.state.data._id !== this.props.selectedRole._id
    ) {
      await this.populateRoles();
    }
    if (prevProps.formDisplay !== this.props.formDisplay) {
      this.setState({ formDisplay: this.props.formDisplay });
    }
  }

  mapToViewModel(role) {
    return {
      _id: role._id,
      nom: role.nom,
    };
  }
  doSubmit = async () => {
    let { data } = this.state;
    await saveRole(data);
    this.setState({ data: { nom: "" } });
    this.props.updateData();
  };

  render() {
    return (
      <>
        {this.props.formDisplay ? (
          <div className="mt-1 h-[fit-content] w-full rounded-tr-md border border-white bg-white shadow-md">
            <p className="ml-2 mt-2 w-full text-xl font-bold text-[#474a52]">
              Formulaire du rôle
            </p>
            <form
              className="mb-6 ml-2 mr-2.5 mt-2 flex w-[100%] flex-wrap justify-start"
              onSubmit={this.handleSubmit}
            >
              {this.renderInput("nom", "Nom du rôle")}
              {this.renderButton("Sauvegarder")}
            </form>
          </div>
        ) : (
          ""
        )}
      </>
    );
  }
}

export default RoleForm;
